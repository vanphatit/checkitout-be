...
import {
  CreateRouteDto,
  UpdateRouteDto,
  CreateRouteFromAutoDto,
} from './dto/route.dto';
import {
  OpenStreetMapService,
  LocationCoordinates,
} from '../common/services/openstreetmap.service';
...
export class RouteService {
  ...
  async createFromAuto(
    createFromAutoDto: CreateRouteFromAutoDto,
  ): Promise<Route> {
    // Validate stations exist
    ...
    const stations = await this.stationModel
      .find({
        _id: {
          $in: createFromAutoDto.stationIds.map((id) => new Types.ObjectId(id)),
        },
      })
      .exec();
    ...
    try {
      ...
      const routeInfo =
        await this.openStreetMapService.calculateRoute(waypoints);

      const newRoute = new this.routeModel({
        name: createFromAutoDto.name,
        stationIds: createFromAutoDto.stationIds.map(
        ...
        googleRouteData: {
          ...
          legs: routeInfo.legs?.map((leg) => ({
            distance: {
              text: `${leg.distance.toFixed(1)} km`,
              value: leg.distance * 1000,
            },
            duration: {
              text: `${Math.round(leg.duration)} phút`,
              value: leg.duration * 60,
            },
            startLocation: {
      ...
      throw new BadRequestException(
        'Lỗi khi tạo tuyến đường: ' + error.message,
      );
    }
  }
  ...
  async update(id: string, updateRouteDto: UpdateRouteDto): Promise<Route> {
    ...
    if (updateRouteDto.stationIds) {
      (updateData as any).stationIds = updateRouteDto.stationIds.map(
        (id) => new Types.ObjectId(id),
      );
    }
    ...
    try {
      ...
      if (!updatedRoute) {
        throw new NotFoundException(
          'Không tìm thấy tuyến đường sau khi cập nhật',
        );
      }
      ...
      throw new BadRequestException(
        'Lỗi khi tính toán lại tuyến đường: ' + error.message,
      );
    }
  }
  ...
  private async validateStations(stationIds: string[]): Promise<void> {
    ...
    if (stations.length !== stationIds.length) {
      throw new BadRequestException(
        'Một hoặc nhiều trạm không tồn tại hoặc không hoạt động',
      );
    }
  }
}
