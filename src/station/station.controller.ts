...
export class StationController {
  ...
  @ApiResponse({
    status: 201,
    description: 'Trạm đã được tạo thành công',
  ...
  @ApiResponse({
    status: 201,
    description: 'Trạm đã được tạo từ địa chỉ thành công',
    type: StationResponseDto,
  })
  @ApiResponse({
    status: 400,
    description: 'Lỗi khi lấy dữ liệu từ OpenStreetMap',
  })
  @ApiResponse({ status: 403, description: 'Không có quyền truy cập' })
  createFromAddress(
    @Body() createFromAddressDto: CreateStationFromAddressDto,
  ): Promise<StationResponseDto> {
  ...
  @ApiResponse({
    status: 200,
    description: 'Danh sách trạm với phân trang',
    type: PaginatedStationResponseDto,
  })
  findAll(
  ...
  @ApiResponse({
    status: 200,
    description: 'Danh sách trạm gần nhất',
    type: [StationResponseDto],
  })
  findNearby(
  ...
  @ApiResponse({
    status: 200,
    description: 'Danh sách trạm xe buýt gần nhất',
  ...
  findNearbyBusStations(
    ...
    return this.stationService.findNearbyBusStations(
      longitude,
      latitude,
      radius,
    );
  ...
  @ApiResponse({
    status: 200,
    description: 'Kết quả tìm kiếm địa điểm',
    type: [PlaceSearchResponseDto],
  })
  ...
  @ApiResponse({
    status: 200,
    description: 'Khoảng cách giữa 2 trạm (km)',
  ...
  @ApiResponse({
    status: 200,
    description: 'Trạm đã được cập nhật thành công',
    type: StationResponseDto,
  })
  @ApiResponse({ status: 404, description: 'Không tìm thấy trạm' })
  @ApiResponse({ status: 403, description: 'Không có quyền truy cập' })
  ...
  @ApiResponse({
    status: 200,
    description: 'Trạm đã được xóa thành công',
    type: StationResponseDto,
  })
  @ApiResponse({ status: 404, description: 'Không tìm thấy trạm' })
  @ApiResponse({ status: 403, description: 'Không có quyền truy cập' })
