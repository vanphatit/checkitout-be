...
import { SchedulingExcelRowDto, ExcelImportResultDto, ImportSchedulingExcelDto } from '../dto/excel-import.dto';
...
export class ExcelImportService {
     ...
     */
    async importFromExcel(file: Express.Multer.File): Promise<ExcelImportResultDto> {
        try {
            // Parse Excel file
            const rawData = await this.excelProcessingService.parseExcelFile(file.buffer);
            ...
            // Validate Excel data
            const { validRows, errors } = await this.excelProcessingService.validateExcelData(rawData);
     ...
     */
    private async processValidRows(validRows: SchedulingExcelRowDto[]): Promise<Partial<ExcelImportResultDto>> {
        ...
        for (const row of validRows) {
            try {
                ...
                const schedulingData: any = {
                    ...
                    note: row.note,
                    availableSeats: bus.seats?.length || bus.vacancy || bus.seatCount || 30,
                    bookedSeats: 0,
                    status: 'scheduled',
                    isActive: true
                };
     ...
     */
    private calculateArrivalDate(departureDate: Date, estimatedDurationMinutes: number): Date {
