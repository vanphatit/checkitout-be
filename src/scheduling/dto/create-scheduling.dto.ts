...
export class CreateSchedulingDto {
    ...
    @Matches(/^\d{2}:\d{2}$/, { message: 'Giờ khởi hành phải có định dạng HH:mm (ví dụ: 08:00)' })
    ...
    @IsEnum(['scheduled', 'in-transit', 'completed', 'cancelled'], {
        message: 'Trạng thái phải là một trong: scheduled, in-transit, completed, cancelled'
    })
    status?: string;
    ...
    @Matches(/^\d{2}:\d{2}$/, { message: 'Giờ đến phải có định dạng HH:mm (ví dụ: 11:00)' })
    arrivalTime?: string;
...
export class UpdateSchedulingDto {
    ...
    @Matches(/^\d{2}:\d{2}$/, { message: 'Giờ khởi hành thực tế phải có định dạng HH:mm' })
    ...
    @Matches(/^\d{2}:\d{2}$/, { message: 'Giờ đến thực tế phải có định dạng HH:mm' })
