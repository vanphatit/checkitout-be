...
export class CreateSchedulingDto {
    ...
    @ApiProperty({ description: 'Thời gian khởi hành dự kiến (HH:mm)', example: '08:00' })
    @IsString()
    ...
    @ApiProperty({ description: 'Thời gian đến dự kiến (HH:mm)', required: false, example: '12:00' })
    @IsOptional()
    @IsString()
    @Matches(/^([0-1]?[0-9]|2[0-3]):[0-5][0-9]$/, {
    ...
    @ApiProperty({
        ...
        required: false,
        example: { name: 'Nguyễn Văn A', phone: '0123456789', licenseNumber: 'B123456789' }
    })
    @IsOptional()
    driver?: {
        name: string;
...
export class UpdateSchedulingDto extends PartialType(CreateSchedulingDto) {
    ...
    @ApiProperty({ description: 'Thời gian khởi hành thực tế (HH:mm)', required: false })
    @IsOptional()
    @IsString()
    @Matches(/^([0-1]?[0-9]|2[0-3]):[0-5][0-9]$/, {
    ...
    @ApiProperty({ description: 'Thời gian đến thực tế (HH:mm)', required: false })
    @IsOptional()
    @IsString()
    @Matches(/^([0-1]?[0-9]|2[0-3]):[0-5][0-9]$/, {
        message: 'Thời gian thực tế phải có định dạng HH:mm'
