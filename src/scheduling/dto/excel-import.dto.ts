import { IsString, IsDateString, IsNumber, IsOptional, Matches, IsArray, ValidateNested } from 'class-validator';
import { Type, Transform } from 'class-transformer';
import { ApiProperty } from '@nestjs/swagger';

export class SchedulingExcelRowDto {
    @ApiProperty({ description: 'Tên tuyến đường', example: 'Sai Gon - Hong Ngu' })
    ...
    @IsDateString({}, { message: 'Ngày khởi hành không hợp lệ. Format: YYYY-MM-DD' })
    @Transform(({ value }) => {
    ...
    @ApiProperty({ description: 'Giờ đến dự kiến', example: '12:30', required: false })
    @IsOptional()
    ...
    @ApiProperty({ description: 'Tên tài xế', example: 'Nguyễn Văn A', required: false })
    @IsOptional()
    @IsString({ message: 'Tên tài xế phải là chuỗi' })
    driverName?: string;

    @ApiProperty({ description: 'SĐT tài xế', example: '0987654321', required: false })
    @IsOptional()
    ...
    @ApiProperty({ description: 'GPLX tài xế', example: 'B1234567', required: false })
    @IsOptional()
    ...
    @ApiProperty({ description: 'Số hàng trong Excel (để báo lỗi)', required: false })
    @IsOptional()
    @IsNumber()
    rowIndex?: number;
}
