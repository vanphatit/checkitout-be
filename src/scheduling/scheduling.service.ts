...
export class SchedulingService {
  ...
  async create(createSchedulingDto: CreateSchedulingDto): Promise<Scheduling> {
    ...
    const route = await this.routeModel
      .findById(createSchedulingDto.routeId)
      .exec();
    if (!route) {
    ...
    const buses = await this.busModel
      .find({
        _id: {
          $in: createSchedulingDto.busIds.map((id) => new Types.ObjectId(id)),
        },
      })
      .exec();
    ...
    const newScheduling = new this.schedulingModel({
      ...
      recurringEndDate: createSchedulingDto.recurringEndDate
        ? new Date(createSchedulingDto.recurringEndDate)
        : undefined,
    });
  ...
  async createBulk(
    createBulkDto: CreateBulkSchedulingDto,
  ): Promise<Scheduling[]> {
    const { startDate, endDate, recurringDays, ...baseScheduling } =
      createBulkDto;
    ...
    for (const scheduleDto of schedules) {
      try {
        ...
        console.error(
          `Failed to create schedule for ${scheduleDto.departureDate}:`,
          error.message,
        );
      }
    }
  ...
  async update(
    ...
    if (updateSchedulingDto.busIds) {
      ...
      const buses = await this.busModel
        .find({
          _id: {
            $in: updateSchedulingDto.busIds.map((id) => new Types.ObjectId(id)),
          },
        })
        .exec();
      ...
      updateData.availableSeats =
        totalSeats - (currentScheduling?.bookedSeats || 0);
    }
  ...
  async updateSeatCount(id: string, bookedSeats: number): Promise<Scheduling> {
    ...
    const totalSeats = buses.reduce(
      (sum, bus) => sum + (bus.seats?.length || 0),
      0,
    );
  ...
  private async validateBuses(
    ...
    if (buses.length !== busIds.length) {
      throw new BadRequestException(
        'Một hoặc nhiều xe không tồn tại hoặc đang bảo trì',
      );
    }
    ...
    if (conflictingSchedules.length > 0) {
      throw new BadRequestException(
        'Một hoặc nhiều xe đã được lập lịch cho thời gian này',
      );
    }
  }
