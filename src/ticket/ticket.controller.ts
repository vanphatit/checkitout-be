...
export class TicketController {
  ...
  @ApiOperation({
    ...
    description: `
      - Customer: Auto-uses their email from JWT token
      - Admin/Seller: Must provide customer email in request body
  ...
  async create(
    ...
    // Determine customer email based on role
    let customerEmail: string;

    if (userRole === UserRole.CUSTOMER) {
      // Customer creates ticket for themselves
      if (!userEmail) {
        throw new BadRequestException('Customer must have an email registered');
      }
      customerEmail = userEmail;
    } else if (userRole === UserRole.ADMIN || userRole === UserRole.SELLER) {
      // Admin/Seller creates ticket for customer
      if (!dto.email) {
        throw new BadRequestException('Email is required for admin/seller to create ticket');
      }
    ...
    return this.ticketService.create({ ...dto, email: customerEmail });
  }
  ...
  async createAndPay(
    ...
    // 1. Create ticket
    const ticket = await this.ticketService.create({ ...dto, email: customerEmail });
  ...
  @ApiQuery({ name: 'phone', required: false, type: String, description: 'Filter by customer phone' })
  ...
  @ApiOperation({
    ...
    description: 'Returns all tickets belonging to the authenticated user (based on their email)'
  })
  @ApiQuery({ name: 'status', required: false, enum: TicketStatus })
  ...
  @ApiParam({ name: 'email', description: 'Customer email', example: 'customer@example.com' })
  @ApiQuery({ name: 'status', required: false, enum: TicketStatus })
  ...
  @ApiParam({ name: 'phone', description: 'Customer phone', example: '0901234567' })
  @ApiQuery({ name: 'status', required: false, enum: TicketStatus })
  @ApiResponse({ status: 200, description: 'Tickets for customer' })
  getTicketsByPhone(
  ...
  @ApiOperation({
    ...
    description: 'Revenue breakdown for a specific scheduling with occupancy rate and seat details'
  })
  @ApiParam({ name: 'schedulingId', description: 'Scheduling ID' })
  @ApiResponse({
    status: 200,
    ...
    schema: {
      example: {
        ...
        ticketDetails: [
          { seatNo: 'A1', price: 225000, paymentMethod: 'BANKING', paidAt: '2025-08-28T10:30:00.000Z' },
          { seatNo: 'A2', price: 225000, paymentMethod: 'CASH', paidAt: '2025-08-28T11:45:00.000Z' }
        ]
      }
    }
  })
  ...
  @ApiOperation({
    ...
    description: 'Overview of revenue and ticket counts for today, this month, this year, and all time'
  })
  @ApiResponse({
    status: 200,
  ...
  @ApiOperation({
    ...
    description: 'Revenue breakdown for current day with payment method details'
  })
  @ApiResponse({
    status: 200,
...
@ApiOperation({
  ...
  description: 'Get paginated ticket list with analytics filters for UI display'
})
@ApiQuery({
  name: 'period',
  required: false,
...
getTicketsWithAnalytics(
  @Query('period') period?: 'today' | 'thisMonth' | 'thisYear' | 'allTime' | 'custom',
